...
function distributionChart(items: InspectorDistribution[]): string {
  if (!items.length) return `<div class="empty">لا توجد نتائج فعلية لبناء الرسم.</div>`;
...
function skillsChart(items: InspectorSkillAnalysis[]): string {
  if (!items.length) return `<div class="empty">لا توجد نتائج فعلية لتحليل الكفايات.</div>`;
...
function levelComparisonChart(analysis: InspectorAnalysis): string {
  ...
  if (!total) return `<div class="empty">لا توجد نتائج فعلية لمقارنة مستويات التلاميذ.</div>`;
...
function supportTable(analysis: InspectorAnalysis): string {
  ...
  if (!rows.length) return `<div class="empty">${analysis.hasRealResults ? "لا يوجد تلميذ تحت عتبة الدعم المحددة." : "لا توجد نتائج فعلية؛ لا تُنشأ لائحة دعم."}</div>`;
...
export function inspectorReportHtml(report: InspectorReport, analysis: InspectorAnalysis): string {
  const resultNote = analysis.hasRealResults
    ? "تم احتساب المؤشرات أدناه من السجلات الفعلية المحفوظة المرتبطة بالقسم والفترة المحددين."
    : "لا توجد نتائج فعلية محفوظة مطابقة للقسم والفترة ونوع التقويم؛ لذلك لا تُعرض أرقام مُنشأة ولا تُحتسب مؤشرات وهمية.";
