        ...
        <div className="rounded-3xl border border-brand-200/70 bg-white p-5 shadow-[0_22px_55px_-30px_rgba(4,36,26,0.35)] sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-4 font-display text-2xl font-black text-ink-900">{inspectorReportTitle(draft.assessmentType)}</h2>
              <p className="mt-2 max-w-3xl text-sm leading-relaxed text-ink-500">استعمل البيانات التجريبية المعزولة لتطوير {inspectorReportTitle(draft.assessmentType)}، ومعاينته وتصديره. لا تُحفظ نسخة Demo ضمن التقارير المركزية.</p>
            </div>
        ...
        <Reveal delay={80}>
          <form className="rounded-3xl border border-ink-900/6 bg-white p-5 sm:p-6" onSubmit={(event) => { event.preventDefault(); void generate(true); }}>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <button type="submit" disabled={busy !== null || !draft.className} className="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-4 py-3 text-xs font-black text-white shadow-lg shadow-brand-700/20 enabled:hover:bg-brand-700 disabled:opacity-40"><FileBarChart className="size-4" aria-hidden="true" />{busy === "generate" || busy === "save" ? "جارٍ الإنشاء والحفظ…" : (dataMode === "demo" ? "إنشاء تقرير Demo للمعاينة" : `إنشاء ${inspectorReportTitle(draft.assessmentType)}`)}</button>
